...
# Load the dataframe
sm_df = pd.read_pickle('peach2-analytics-dashboard\sentiment_data_vader_labels.pkl')
       ...
       html.Div([
           dcc.Graph(id='compound-sentiment-line-graph', 
                    figure = px.line(sentiment_means, x=sentiment_means['authoredAt'], y=['compound_7day'], labels={'compound_7day':'Average Compound Sentiment'},
                                     title = 'Average Compound Sentiment Over Time <br><sup>Plot Subtitle</sup>'),
                    style = {'border': '2px solid #e8e8e8', 'background-color': '#f8f9fa'}),
                    html.P('The Compound Sentiment is a normalized measure of how positive or negative a post is classified to be. The closer to 1 this normalized value is, the more positive the post sentiment is. The closer to -1 the value is, the more negative the post sentiment is.')
                    ]),
        html.Br(),
...
def line_graphs(platform_list, account_category, account_identity, account_type, account_location, time_frame, relative_date, start_date, end_date):
    ...
    compound_sentiment_line_graph.update_layout( # Update the layout of the figure
        ...
        # annotations = [dict(
        # x=0.5,
        # y=-0.2,    #Trying a negative number makes the caption disappear - I'd like the caption to be below the map
        # xref='paper',
        # yref='paper',
        # text='',
        # showarrow = False
    ...
    return compound_sentiment_line_graph, posts_line_graph, sentiment_pie_chart
